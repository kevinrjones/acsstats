...
public class
    BowlingRecordsCompleteQuery : IRequest<Result<SqlResultEnvelope<IReadOnlyList<BowlingCareerRecordDto>>, Error>>
{
    private BattingBowlingFieldingModel FieldingModel { get; }
    private string Sql { get; }

    public BowlingRecordsCompleteQuery(BattingBowlingFieldingModel fieldingModel, string sql)
    {
        FieldingModel = fieldingModel;
        Sql = sql;
    }

    internal class BowlingRecordsCompleteQueryHandler
        : IRequestHandler<BowlingRecordsCompleteQuery,
            Result<SqlResultEnvelope<IReadOnlyList<BowlingCareerRecordDto>>, Error>>
    {
        private readonly QueriesConnectionString _queriesConnectionString;
        private readonly ILogger<BowlingRecordsCompleteQueryHandler> _logger;

        public BowlingRecordsCompleteQueryHandler(
            QueriesConnectionString queriesConnectionString,
            ILogger<BowlingRecordsCompleteQueryHandler> logger)
        {
            _queriesConnectionString = queriesConnectionString;
            _logger = logger;
        }

        public async Task<Result<SqlResultEnvelope<IReadOnlyList<BowlingCareerRecordDto>>, Error>> Handle(
            BowlingRecordsCompleteQuery request,
            CancellationToken cancellationToken)
        {
            ...
            catch (Exception e)
            {
                ...
                return Result.Failure<SqlResultEnvelope<IReadOnlyList<BowlingCareerRecordDto>>, Error>(
