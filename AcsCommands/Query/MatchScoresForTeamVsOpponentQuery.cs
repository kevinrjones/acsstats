...
public class MatchScoresForTeamVsOpponentQuery : IRequest<Result<IReadOnlyList<MatchRecordDetailsDto>, Error>>
{
    private SharedModel Model { get; }
    private string Sql { get; }

    public MatchScoresForTeamVsOpponentQuery(SharedModel model, string sql)
    {
        Model = model;
        Sql = sql;
    }

    internal class MatchScoresForTeamVsOpponentQueryHandler
        : IRequestHandler<MatchScoresForTeamVsOpponentQuery, Result<IReadOnlyList<MatchRecordDetailsDto>, Error>>
    {
        private readonly QueriesConnectionString _queriesConnectionString;
        private readonly ILogger<MatchScoresForTeamVsOpponentQueryHandler> _logger;

        public MatchScoresForTeamVsOpponentQueryHandler(
            QueriesConnectionString queriesConnectionString,
            ILogger<MatchScoresForTeamVsOpponentQueryHandler> logger)
        {
            _queriesConnectionString = queriesConnectionString;
            _logger = logger;
        }

        public async Task<Result<IReadOnlyList<MatchRecordDetailsDto>, Error>> Handle(MatchScoresForTeamVsOpponentQuery request,
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    team_id = request.Model.OpponentsId.Value,
