...
public class MatchResultCompleteQuery : IRequest<Result<IReadOnlyList<MatchResultDto>, Error>>
{
    private SharedModel Model { get; }
    private string Sql { get; }

    public MatchResultCompleteQuery(SharedModel model, string sql)
    {
        Model = model;
        Sql = sql;
    }

    internal class MatchResultCompleteQueryHandler
        : IRequestHandler<MatchResultCompleteQuery, Result<IReadOnlyList<MatchResultDto>, Error>>
    {
        private readonly QueriesConnectionString _queriesConnectionString;
        private readonly ILogger<MatchResultCompleteQueryHandler> _logger;

        public MatchResultCompleteQueryHandler(
            QueriesConnectionString queriesConnectionString,
            ILogger<MatchResultCompleteQueryHandler> logger)
        {
            _queriesConnectionString = queriesConnectionString;
            _logger = logger;
        }

        public async Task<Result<IReadOnlyList<MatchResultDto>, Error>> Handle(MatchResultCompleteQuery request, CancellationToken cancellationToken)
        {
            try
            {
