...
public class MatchScoresVsOpponentQuery : IRequest<Result<IReadOnlyList<MatchRecordDetailsDto>, Error>>
{
    private SharedModel Model { get; }
    private string Sql { get; }

    public MatchScoresVsOpponentQuery(SharedModel model, string sql)
    {
        Model = model;
        Sql = sql;
    }

    internal class MatchScoresVsOpponentQueryHandler
        : IRequestHandler<MatchScoresVsOpponentQuery, Result<IReadOnlyList<MatchRecordDetailsDto>, Error>>
    {
        private readonly QueriesConnectionString _queriesConnectionString;
        private readonly ILogger<MatchScoresVsOpponentQueryHandler> _logger;

        public MatchScoresVsOpponentQueryHandler(
            QueriesConnectionString queriesConnectionString,
            ILogger<MatchScoresVsOpponentQueryHandler> logger)
        {
            _queriesConnectionString = queriesConnectionString;
            _logger = logger;
        }

        public async Task<Result<IReadOnlyList<MatchRecordDetailsDto>, Error>> Handle(MatchScoresVsOpponentQuery request,
