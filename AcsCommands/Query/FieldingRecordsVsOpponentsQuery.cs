...
public class FieldingRecordsVsOpponentsQuery : IRequest<Result<IReadOnlyList<FieldingCareerRecordDto>, Error>>
{
    private BattingBowlingFieldingModel FieldingModel { get; }
    private string Sql { get; }

    public FieldingRecordsVsOpponentsQuery(BattingBowlingFieldingModel fieldingModel, string sql)
    {
        FieldingModel = fieldingModel;
        Sql = sql;
    }

    internal class FieldingRecordsVsOpponentsQueryHandler
        : IRequestHandler<FieldingRecordsVsOpponentsQuery, Result<IReadOnlyList<FieldingCareerRecordDto>, Error>>
    {
        private readonly QueriesConnectionString _queriesConnectionString;
        private readonly ILogger<FieldingRecordsVsOpponentsQueryHandler> _logger;

        public FieldingRecordsVsOpponentsQueryHandler(
            QueriesConnectionString queriesConnectionString,
            ILogger<FieldingRecordsVsOpponentsQueryHandler> logger)
        {
            _queriesConnectionString = queriesConnectionString;
            _logger = logger;
        }

        public async Task<Result<IReadOnlyList<FieldingCareerRecordDto>, Error>> Handle(FieldingRecordsVsOpponentsQuery request,
