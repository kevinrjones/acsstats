...
public class PartnershipRecordsForTeamQuery :
    IRequest<Result<IReadOnlyList<PartnershipCareerRecordDetailsDto>, Error>>
{
    private PartnershipModel Model { get; }
    private string Sql { get; }

    public PartnershipRecordsForTeamQuery(PartnershipModel model, string sql)
    {
        Model = model;
        Sql = sql;
    }

    internal class PartnershipRecordsForTeamQueryHandler
        : IRequestHandler<PartnershipRecordsForTeamQuery,
            Result<IReadOnlyList<PartnershipCareerRecordDetailsDto>, Error>>
    {
        private readonly QueriesConnectionString _queriesConnectionString;
        private readonly ILogger<PartnershipRecordsForTeamQueryHandler> _logger;

        public PartnershipRecordsForTeamQueryHandler(
            QueriesConnectionString queriesConnectionString,
            ILogger<PartnershipRecordsForTeamQueryHandler> logger)
        {
        ...
        public async Task<Result<IReadOnlyList<PartnershipCareerRecordDetailsDto>, Error>> Handle(
            PartnershipRecordsForTeamQuery request, CancellationToken cancellationToken)
        {
            try
            {
