...
public class BattingRecordsForTeamVsOpponentsQuery : IRequest<Result<IReadOnlyList<BattingCareerRecordDto>, Error>>
{
    private BattingBowlingFieldingModel FieldingModel { get; }
    private string Sql { get; }

    public BattingRecordsForTeamVsOpponentsQuery(BattingBowlingFieldingModel fieldingModel, string sql)
    {
        FieldingModel = fieldingModel;
        Sql = sql;
    }

    internal class BattingRecordsForTeamVsOpponentsQueryHandler
        : IRequestHandler<BattingRecordsForTeamVsOpponentsQuery, Result<IReadOnlyList<BattingCareerRecordDto>, Error>>
    {
        private readonly QueriesConnectionString _queriesConnectionString;
        private readonly ILogger<BattingRecordsForTeamVsOpponentsQueryHandler> _logger;

        public BattingRecordsForTeamVsOpponentsQueryHandler(
            QueriesConnectionString queriesConnectionString,
            ILogger<BattingRecordsForTeamVsOpponentsQueryHandler> logger)
        {
            _queriesConnectionString = queriesConnectionString;
            _logger = logger;  
        }
        
        public async Task<Result<IReadOnlyList<BattingCareerRecordDto>, Error>> Handle(BattingRecordsForTeamVsOpponentsQuery request, CancellationToken cancellationToken)
        {
            try
            {
